...
public abstract class BaseTankEntity extends BaseEntity implements Controllable {
    ...
    public void render(GraphicsContext gc, GameContext context) {
        ...
        int direction = (int) (rotation / (Math.PI / 2)) % 4;
        String[] imgNameS1 = { "tank_base", "tank_base1", "tank_base2", "tank_base3" };
        String[] imgNameS2 = { "aitank_base1", "aitank_base2", "aitank_base3", "aitank_base4" };
        direction = (direction + 4) % 4;
        String imgName;
        if (teamIndex == 0) {
            imgName = imgNameS1[direction];
        } else {
            imgName = imgNameS2[direction];
        }

        Image BaseImg = context.getImageManager().getImage(imgName);
        gc.drawImage(BaseImg, x, y, width, height);
    ...
    public boolean intersects(Entity other) {
        ...
        boolean result = x < other.getX() + other.getWidth() &&
                x + width > other.getX() &&
